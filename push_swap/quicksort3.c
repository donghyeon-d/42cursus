...
int quick_b(t_stack *stack_a, t_stack *stack_b, int len)
{
	...
	{
		if (stack_a->data[stack_a->top] <= p2)
...
int	quick_a(t_stack *stack_a, t_stack *stack_b, int len)
{
	...
	if (sort->arr == NULL)
		return (FALSE);
	if (sort->len < 3)
		small_sort_b(stack_a, len)
	while (len-- && sort->len > 2)
	{
		if (stack_b->data[stack_b->top] > p1)
			sort->pa += pa(stack_b, stack_b);
		else
			sort->rb += rb(stack_b);
	...
	oper_rra(stack_b, stack_a, sort);
	quick_a(stack_a, stack_b, sort->pa - sort->ra);
	quick_a(stack_a, stack_b, sort->rb);
	free(sort->arr);
	return (sort->pa);
}